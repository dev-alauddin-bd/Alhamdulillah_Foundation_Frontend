    ...
    <div className="min-h-screen bg-background pt-40 pb-16">
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12 text-center max-w-2xl mx-auto">
          <AFSectionTitle
            title={t("noticesPage.officialNotices", { defaultValue: "Official Notices" })}
            subtitle={t("noticesPage.subtitle", { defaultValue: "Important Announcements & Updates" })}
            align="center"
          />
          <p className="text-muted-foreground mt-4">
             {t("noticesPage.description", { defaultValue: "Stay informed about the latest activities, decisions, and reports from the Alhamdulillah Foundation board." })}
          </p>
        </div>
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            {notices.length > 0 ? (
                  ...
                  <CardContent className="p-0 flex flex-col sm:flex-row items-center">
                    ...
                    <div className="flex-1 p-5 text-center sm:text-left">
                       ...
                       <div className="flex items-center justify-center sm:justify-start gap-2 text-xs text-muted-foreground">
                          ...
                          <span>{t("noticesPage.officialDocument", { defaultValue: "Official Document" })}</span>
                          {notice.submitBy && (
                             <>
                                <span className="w-1 h-1 rounded-full bg-border" />
                                <span>{t("noticesPage.postedBy", { name: notice.submitBy.name, defaultValue: `Posted by ${notice.submitBy.name}` })}</span>
                             </>
                          )}
                       </div>
                    </div>
                       ...
                       {notice.fileUrl ? (
                          <Button asChild variant="outline" className="rounded-full gap-2 border-primary/20 hover:bg-primary hover:text-white group-hover:border-white/20">
                             <a href={notice.fileUrl} target="_blank" rel="noopener noreferrer">
                                ...
                                {t("noticesPage.download", { defaultValue: "Download" })}
                             </a>
                          </Button>
                       ) : (
                          <span className="text-xs text-muted-foreground italic">{t("noticesPage.noAttachment", { defaultValue: "No Attachment" })}</span>
                       )}
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="text-center py-16 bg-card/30 rounded-2xl border-2 border-dashed border-border/50">
                 ...
                 <h3 className="text-xl font-bold mb-2">{t("noticesPage.noNoticesFound", { defaultValue: "No Notices Found" })}</h3>
                 <p className="text-muted-foreground">{t("noticesPage.noNoticesDesc", { defaultValue: "There are no notices to display at this time." })}</p>
              </div>
            )}
          </div>
        )}
